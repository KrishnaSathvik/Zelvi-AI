import { useTheme } from '../../contexts/ThemeContext'

export default function ThemeSwitcher() {
  const { theme, setTheme } = useTheme()

  return (
    <div className="border-2 p-6 mb-6 rounded-sm backdrop-blur-modern theme-bg-form theme-border">
      <h2 className="text-xl font-semibold mb-4 font-mono uppercase theme-text-main">Theme</h2>
      <div className="flex items-center gap-2">
        <button
          onClick={() => setTheme('light')}
          className={`flex items-center justify-center w-12 h-12 rounded-lg border-2 transition-all ${
            theme === 'light'
              ? 'border-cyan-400 bg-cyan-400/10 shadow-[0_0_0_2px_rgb(0,217,255)]'
              : 'hover:border-cyan-400/50 bg-transparent'
          }`}
          style={theme !== 'light' ? {
            borderColor: 'var(--border-main)',
            color: 'var(--text-main)'
          } : undefined}
          aria-label="Light theme"
        >
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="theme-text-main"
          >
            <circle cx="12" cy="12" r="5" />
            <line x1="12" y1="1" x2="12" y2="3" />
            <line x1="12" y1="21" x2="12" y2="23" />
            <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
            <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
            <line x1="1" y1="12" x2="3" y2="12" />
            <line x1="21" y1="12" x2="23" y2="12" />
            <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
            <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
          </svg>
        </button>

        <button
          onClick={() => setTheme('dark')}
          className={`flex items-center justify-center w-12 h-12 rounded-lg border-2 transition-all ${
            theme === 'dark'
              ? 'border-cyan-400 bg-cyan-400/10 shadow-[0_0_0_2px_rgb(0,217,255)]'
              : 'hover:border-cyan-400/50 bg-transparent'
          }`}
          style={theme !== 'dark' ? {
            borderColor: 'var(--border-main)',
            color: 'var(--text-main)'
          } : undefined}
          aria-label="Dark theme"
        >
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="theme-text-main"
          >
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
          </svg>
        </button>

        <button
          onClick={() => setTheme('system')}
          className={`flex items-center justify-center w-12 h-12 rounded-lg border-2 transition-all ${
            theme === 'system'
              ? 'border-cyan-400 bg-cyan-400/10 shadow-[0_0_0_2px_rgb(0,217,255)]'
              : 'hover:border-cyan-400/50 bg-transparent'
          }`}
          style={theme !== 'system' ? {
            borderColor: 'var(--border-main)',
            color: 'var(--text-main)'
          } : undefined}
          aria-label="System theme"
        >
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="theme-text-main"
          >
            <rect x="2" y="3" width="20" height="14" rx="2" ry="2" />
            <line x1="8" y1="21" x2="16" y2="21" />
            <line x1="12" y1="17" x2="12" y2="21" />
          </svg>
        </button>
      </div>
    </div>
  )
}

